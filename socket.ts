...
export default class AsyncParamsPhoenixSocket extends Socket {
  ...
  connect(_params: SocketParams): void {
    if (this.params?.constructor == Function && (this.params as SockParamsPromise)()?.constructor === Promise) {
      Promise.resolve((this.params as SockParamsPromise)()).then((params) => super.connect(params));
    } else {
      super.connect();
    }
  }
